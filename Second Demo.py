    ...
    for contour in contours:
        ...
        if len(approx) == 6:
            if moments["m00"] != 0:
                ...
                Cy, Cx = 0, 0
            cv2.putText(final_image, "Place Purple Pixel here", (Cx, Cy), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
        if len(approx) == 10:
            if moments["m00"] != 0:
                Cx = int(moments["m10"] / moments["m00"])
                Cy = int(moments["m01"] / moments["m00"])      
...
while True:
    ...
    color_img[thresh == 255] = frame[thresh == 255]
    resized_image = cv2.bitwise_and(frame, color_img)
    k = 7
    ...
    result = cv2.bitwise_or(green_result, yellow_result)
            ...
            for j in range(y, y + h):
                if mask[j, i] == 255 and gray_image[j, i] == 0:  # If pixel is inside contour and black
                    ...
                    if all(n != 2 for n in neighbors): 
                        result[j, i] = [result[j-1, i, 0], result[j-1, i, 1], result[j-1, i, 2]]


    cv2.imshow('Captured Frame', frame)
    cv2.imshow('sdf', result)
    cv2.imshow('sdfsa', yellow_result)        
...
# Purple Color
x = 25
purple_image, p = group_colors(resized_image, x)
closest_purple = find_green(p, purple)
purple_mask = cv2.inRange(purple_image, closest_purple, closest_purple)
purple_result = cv2.bitwise_or(purple_image, purple_image, mask=purple_mask)
gray = cv2.cvtColor(purple_result, cv2.COLOR_BGR2GRAY)
contours, hierarchy = cv2.findContours(gray, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
for contour in contours:
    approx = cv2.approxPolyDP(contour, 0.01 * cv2.arcLength(contour, True), True)
    x = approx.ravel()[0]
    y = approx.ravel()[1]
    cv2.drawContours(purple_result, [approx], 0, (184, 134, 157), 10)

green_and_yellow = cv2.bitwise_or(green_result, yellow_result)
result = cv2.bitwise_or(green_and_yellow, purple_result)
gray_image = cv2.cvtColor(result, cv2.COLOR_BGR2GRAY)
_, thresholded = cv2.threshold(gray_image, 10, 255, cv2.THRESH_BINARY)
contours, _ = cv2.findContours(thresholded, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
for contour in contours:
...
# Shape Detection
demo = pixel_detector(result, resized_image, closest_green, closest_purple, closest_yellow)
