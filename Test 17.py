...
# Image Processing
resize = cv2.imread('IMG_2264.webp')
...
closest_yellow = find_green(y, purple)
...
gray_image = cv2.cvtColor(yellow_result, cv2.COLOR_BGR2GRAY)
        ...
        for j in range(y, y + h):
            if mask[j, i] == 255 and gray_image[j, i] == 0:  # If pixel is inside contour and black
                ...
                if all(n != 2 for n in neighbors): 
                    yellow_result[j, i] = [yellow_result[j-1, i, 0], yellow_result[j-1, i, 1], yellow_result[j-1, i, 2]]

# Shape Detection
demo = pixel_detector(yellow_result, resized_image, green,yellow, closest_yellow)

# Displaying Results
cv2.imshow('Original Image', yellow_result)
